...
protected:
	...
	basic_genetic(int pop_size, int chance):
			population_(pop_size), chance_(chance), gen_idx(0),
			// Stats
			max_(0), sum_(0), avg_(0.0), mad_(0.0), data_(pop_size),
			mid_gen(pop_size + pop_size % 2), fitness(pop_size), pool(""){};
	basic_genetic(int pop_size, int chance, int str_size, std::string p);

	virtual char generate(int r) const { return pool[r % pool.size()]; }
